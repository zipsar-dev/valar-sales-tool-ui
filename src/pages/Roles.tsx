...
import { Search, Filter, Edit, ChevronLeft, ChevronRight, Eye, Shield, Trash2 } from 'lucide-react';
    ...
    <Card className="p-3 sm:p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center space-x-3 sm:space-x-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 truncate">{role.role_key}</p>
        </div>
      </div>

      <div className="mt-2 sm:mt-3 flex flex-wrap gap-1">
        {role.permissions.slice(0, 2).map((perm) => (
          <span key={perm} className="inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-xs font-medium bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300">
            {perm}
          </span>
        ))}
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
        ...
        <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-700">
          {roles.map((role) => (
            <tr key={role.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800">
                ...
                <div>
                  ...
                  <div className="mt-1 sm:mt-2 flex flex-wrap gap-1 sm:gap-2">
                    {selectedRole.permissions.map((perm) => (
                      <span key={perm} className="inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-xs font-medium bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200">
                        {perm}
                      </span>
                    ))}
                  </div>
      ...
      <EditRoleModal
        ...
        onSave={async (data) => {
          ...
          try {
            ...
            if (data.addedPermissions?.length || data.removedPermissions?.length) {
