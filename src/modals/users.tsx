...
export const EditUserModal: React.FC<EditUserModalProps> = ({ isOpen, onClose, user, onSave }) => {
    ...
    const fetchRoles = async () => {
        try {
            ...
            setPossibleRoles(data.map((role: { role_name: string }) => role.role_name));
        } catch (error) {
    ...
    useEffect(() => {
        if (user) {
            const initial = {
                ...
                roles: user.roles, // Assuming user.roles contains role_name values
            };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <Card className="max-w-lg w-full mx-4 max-h-[90vh] flex flex-col">
                ...
                <div className="flex border-b border-neutral-200 dark:border-neutral-700">
                    <button
                        className={`flex-1 py-2 px-4 text-center ${tab === 'basic' ? 'border-b-2 border-primary-600 text-primary-600' : 'text-neutral-500 dark:text-neutral-400'}`}
                        onClick={() => setTab('basic')}
                    >
                        Basic Info
                    </button>
                    <button
                        className={`flex-1 py-2 px-4 text-center ${tab === 'roles' ? 'border-b-2 border-primary-600 text-primary-600' : 'text-neutral-500 dark:text-neutral-400'}`}
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6" id="editUserForm">
                    ...
                    {tab === 'roles' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <h3 className="text-sm font-medium mb-2 text-neutral-700 dark:text-neutral-300">Available Roles</h3>
                                <div className="space-y-2">
                                    {availableRoles.map((role) => (
                                        <div key={role} className="flex items-center justify-between p-2 border rounded border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                            ...
                            <div>
                                <h3 className="text-sm font-medium mb-2 text-neutral-700 dark:text-neutral-300">Assigned Roles</h3>
                                <div className="space-y-2">
                                    {roles.map((role) => (
                                        <div key={role} className="flex items-center justify-between p-2 border rounded border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                                            <span className="text-neutral-900 dark:text-white">{role}</span>
